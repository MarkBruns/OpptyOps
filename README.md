...
It's important dwell a bit on the achievement that Aligned news is. It simple and just works, but it **represents INFORMED judgment about which knowledgeable experts are actually the most important in moving the DISCIPLINES involved in AI**. [Aligned News for AI](https://alignednews.com/) is built on two decades worth of 63 expert-curated X lists, which index 100,000+ accounts and the interactions amongst those moving the field. In every sweep of data collection, an XPlugin pulls 3,000–7,000 new posts from the priority lists. Then a hyper-efficient PROVEN XPlugin MySQL server interface [written in C++, NOT the typical half-baked, never refactored, high-level abstract javascropt junk-adders found in much of the vibe-coded GitHub codepiles] accomplishes the data wrangling between different systems. Every post is indexed to a [Weaviate vector database](https://docs.weaviate.io/weaviate) with full metadata: text, author, engagement metrics, and timestamp. Seen/unseen tracking ensures only new posts are processed. Only a relatively TEENY set of extremely high quality data actually needs to be processed/distilled/published as follows:
