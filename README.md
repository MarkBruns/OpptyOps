...
[Aligned News for AI](https://alignednews.com/) is built on 63 hand-curated X lists, representing **informed judgment about which people are actually most important in moving the field**. Together these 63 lists index 100,000+ accounts and the interactions amongst those moving the field. In every sweep of data collection, an XPlugin pulls 3,000–7,000 new posts from the priority lists. The importance of the XPlugin interface between the MySQL server and its clients, stems from how it modernizes legacy systems and bridges different development workflows. Every post is indexed to a [Weaviate vector database](https://docs.weaviate.io/weaviate) with full metadata: text, author, engagement metrics, and timestamp. Seen/unseen tracking ensures only new posts are processed. Then the automated distillation process happens:
